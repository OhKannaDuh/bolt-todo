...
function TimeModule:new(plugin)
    TimeModule.super:new(plugin)
...
function TimeModule:key()
    return 'time'
end

function TimeModule:load_data_from_config(config)
...
function TimeModule:get_save_data()
    return {
        year = self.year,
        month = self.month,
        day = self.day,
...
function TimeModule:tick(delta)
...
function TimeModule:add_on_year_changed_callback(callback)
    table.insert(self.on_year_changed_callbacks, callback)
end

function TimeModule:add_on_month_changed_callback(callback)
    table.insert(self.on_year_changed_callbacks, callback)
end

function TimeModule:add_on_weekly_reset_day_callback(callback)
    table.insert(self.on_weekly_reset_day_callbacks, callback)
end

function TimeModule:add_on_day_changed_callback(callback)
    table.insert(self.on_day_changed_callbacks, callback)
end

function TimeModule:add_on_hour_changed_callback(callback)
    table.insert(self.on_hour_changed_callbacks, callback)
end

function TimeModule:add_on_minute_changed_callback(callback)
